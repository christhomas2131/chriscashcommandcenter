"""
company_watcher.py — Scrape current openings from specific target company career pages.

Supported ATS platforms:
  - Breezy HR  : direct JSON API (no auth)
  - Lever      : public REST API (no auth)
  - Workday    : hidden-but-public REST API (no auth)
  - iCIMS      : keyword HTML scrape (no auth)
  - kpmg       : kpmguscareers.com keyword HTML scrape
  - deloitte   : apply.deloitte.com keyword HTML scrape

Results are filtered to roles matching ROLE_KEYWORDS before being returned.
Roles containing federal contract / clearance indicators are always excluded.
"""
...
# Titles/descriptions containing any of these are excluded regardless of keyword match
_CLEARANCE_EXCLUDE = [
    "clearance",
    "ts/sci",
    "top secret",
...
def scrape_workday(company_name: str, tenant: str, wd_num: int, site: str) -> list[dict]:
    ...
    This is an undocumented but publicly accessible REST API used by Workday's
    candidate-facing job board. No authentication required.
    """
...
# ---------------------------------------------------------------------------
# iCIMS  (HTML scrape with keyword search)
# ---------------------------------------------------------------------------

def scrape_icims(company_name: str, subdomain: str) -> list[dict]:
    """
    Searches iCIMS via keyword URL parameters and parses the HTML response.
    Runs one request per keyword group to keep volume low.
    """
    base = f"https://{subdomain}.icims.com"

    # Search with a couple of broad terms to cover our keyword list
    search_terms = [
        "disaster recovery",
        "emergency management",
        "program manager",
    ...
    for term in search_terms:
        url = (
            f"{base}/jobs/search"
            f"?ss=1&searchKeyword={requests.utils.quote(term)}"
            f"&pr=50&in_iframe=1"
        )
        try:
            resp = requests.get(url, headers=_HEADERS, timeout=15)
        ...
        # iCIMS renders job rows as <tr> elements; each has a title <a> and location span
        # Pattern: job title link and location extracted via regex
        row_blocks = re.findall(
            r'class="[^"]*iCIMS_JobsTable[^"]*".*?</tr>',
            html, re.DOTALL
        )
        if not row_blocks:
            # Fallback: extract all title links from iCIMS job listing pages
            row_blocks = re.findall(
                r'<a[^>]+href="([^"]+/jobs/\d+/[^"]+)"[^>]*>(.*?)</a>',
                html, re.DOTALL
            )
        ...
        for block in row_blocks:
            ...
            loc_m = re.search(
                r'class="[^"]*jobAttribute[^"]*"[^>]*>(.*?)</(?:td|span|div)>',
                block, re.DOTALL
            )
            if not title_m:
                continue
...
def _parse_kpmg_card(raw_html: str) -> tuple[str, str]:
    """Extract (title, location) from a KPMG job card's inner HTML."""
    text = re.sub(r"<[^>]+>", "\n", raw_html)
...
def scrape_kpmg(company_name: str, search_terms: list[str]) -> list[dict]:
    """Keyword search on kpmguscareers.com, parse HTML job cards."""
    ...
    for term in search_terms:
        ...
        while True:
            ...
            # Each job is an <a href="/jobdetail/?jobId=..."> block
            cards = re.findall(
            ...
            # Stop paginating if no next-page link or nothing new
...
def scrape_deloitte(company_name: str, search_terms: list[str]) -> list[dict]:
    """
    Keyword search on apply.deloitte.com/en_US/careers/SearchJobs/{term}.
    Parses server-rendered HTML: <h3><a href="...">title</a></h3> with
    location in the following <p> tag.
    """
    ...
    for term in search_terms:
        ...
        # Job cards: <h3><a href="/en_US/careers/JobDetail/...">Title</a></h3>
        # Location nearby in: <p>Deloitte US | Business | Location</p>
        # Capture pairs by scanning for h3+a blocks with adjacent <p>
        blocks = re.findall(
        ...
        # Fallback: just grab all JobDetail links + titles
        if not blocks:
            links = re.findall(
        ...
        for href, title, after_html in blocks:
            ...
            # Extract location from first <p> in the following block
...
def run_company_watcher(companies: list[dict]) -> list[dict]:
    """
    Run all configured company watchers. Returns flat list of job dicts.

    Each entry in `companies` must have at minimum:
      {"name": "...", "ats": "breezy|lever|workday|icims|kpmg|deloitte", ...ats-specific keys...}
    """
    all_jobs: list[dict] = []
